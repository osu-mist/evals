...
public class BackendMgr {
    ...
    public boolean handleAnnualCreation(Job job) throws Exception
    {
        ...
        DateTime createForDate = getCreateForDate();
        String msg;
     ...
     */
    private DateTime getCreateForDate()
    {

        Configuration config = configMap.get("firstGoalDueReminder");
    ...
    private boolean shouldUpdateSalaryInfo(Appraisal appraisal) throws Exception{
        DateTime dueDate = EvalsUtil.getDueDate(appraisal, configMap.get("appraisalDue"));
    ...
    private void updateAppraisals() throws Exception
    {
        ...
        for (int i = 0; i < ids.length; i++)
        {
            try
            {
                ...
                if (newStatus != null) {
                    tx.commit();
                    continue;  //done with this appraisal record
                }

                //if we get here, there is no need to change status.
                // Do we need to send additional reminder email?
                String key = appraisal.getStatus() + "Frequency";
                Configuration frequencyConfig = configMap.get(key);
                if (frequencyConfig != null)   //May need to send followup email
                {
                    Email lastEmail = EmailMgr.getLastEmail(ids[i], status);
                    ...
                    sendMail(appraisal);
                    System.out.println("Sent follow-up email for " + status);
                    followupEmailCount++;
                }
                tx.commit();
            }catch(Exception e)
   ...
   public void updateAppraisal(Appraisal appraisal, String newStatus) throws Exception
    ...
    private boolean shouldSendITWithHoldWarningEmail(Appraisal appraisal) throws Exception {
        ...
        Configuration config = configMap.get("IT-increase-withhold-warn1-days");
        int daysBeforeWarning1 = config.getIntValue();
        config = configMap.get("IT-increase-withhold-warn2-days");
