...
-- ------------------------------------------------------
  ...
  CREATE TABLE `APPRAISALS` (
    ...
    `SUPERVISOR_REBUTTAL_READ` DATETIME,
    `EVALUATION_SUBMIT_DATE` DATETIME,
    `GRANT_DRAFT_RESULTS_PERMISSION` INT,
    `CLOSEOUT_REASON_ID` BIGINT,
    `OVERDUE` SMALLINT, `GOALS_OVERDUE` SMALLINT,
    `GOALS_APPROVAL_OVERDUE` SMALLINT,
    `RESULTS_OVERDUE` SMALLINT,
    `APPRAISAL_OVERDUE` SMALLINT,
    `REVIEW_OVERDUE` SMALLINT,
    `RELEASE_OVERDUE` SMALLINT,
...
-- ------------------------------------------------------
  ...
  CREATE TABLE `PYVPASJ` (
    ...
    `PYVPASJ_PCLS_CODE` VARCHAR(5) NOT NULL COMMENT 'Position class',
    `PYVPASJ_SAL_GRADE` VARCHAR(5) COMMENT 'Salary grade',
    `PYVPASJ_SAL_STEP` SMALLINT COMMENT 'Salary step',
    `PYVPASJ_ORGN_CODE_TS` VARCHAR(6) NOT NULL COMMENT 'Organization code',
    `PYVPASJ_ORGN_DESC` VARCHAR(30) COMMENT 'Organization description',
    `PYVPASJ_BCTR_TITLE` VARCHAR(4) COMMENT 'Business center title',
    `PYVPASJ_SUPERVISOR_PIDM` INT,
    `PYVPASJ_SUPERVISOR_POSN` VARCHAR(6),
    `PYVPASJ_SUPERVISOR_SUFF` VARCHAR(2) COMMENT 'Supervisor job suffix',
...
-- ------------------------------------------------------
  ...
  CREATE TABLE `PYVPDES` (
    ...
    `FLSASTATUS` TEXT COMMENT 'Fair Labor Standards Act status',
    `JOBLOCATION` TEXT,
    `POSITIONCODEDESCRIPTION` TEXT,
    `POSITIONSUMMARY` TEXT,
    `DECISIONMAKINGGUIDELINES` TEXT,
    `PERCLEADWORKSUPERDUTIES` TEXT COMMENT 'Percent of time lead work',
    `NBREMPLLEADORSUPVD` TEXT COMMENT 'Number of employees supervised',
    `POSITIONDUTIES` TEXT,
    `POSITIONDUTIESCONTINUED` TEXT,
    `ADDTLREQQUALIFS` TEXT COMMENT 'Additional qualifications',
    `PREFERREDQUALIFICATIONS` TEXT,
    `CRIMBCKGRNDANDORDMVCHKRQRD` TEXT COMMENT 'Security sensitive position. Criminal background check and or dmv check required.',
