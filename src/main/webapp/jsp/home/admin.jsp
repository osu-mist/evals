...
<%
    ...
    PortletURL bulkUpdateURL = renderResponse.createRenderURL();
    bulkUpdateURL.setWindowState(WindowState.MAXIMIZED);
    bulkUpdateURL.setParameter("action", "list");
    bulkUpdateURL.setParameter("controller", "BulkUpdateAction");
%>
...
<c:if test="${isAdmin == 'true'}">
    <div id="<portlet:namespace/>accordionMenuPassAdmin" class="accordion-menu">
        <div class="osu-accordion-header" onclick="<portlet:namespace/>toggleContent('<portlet:namespace/>passAdmin');">
          <img id="<portlet:namespace/>passAdminImageToggle" src="/o/evals/images/accordion/accordion_arrow_up.png"/>
        ...
        <div class="accordion-content" id="<portlet:namespace/>passAdmin" style="display: block;">
            <ul class="pass-menu-list">
                ...
                <li>
                    <a href="<%= bulkUpdateURL.toString() %>">
                        <liferay-ui:message key="bulk-update-title"/></a>
                </li>
            </ul>
        </div>
    </div>
